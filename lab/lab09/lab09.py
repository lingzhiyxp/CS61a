...
def is_bst(t):
    ...
    def helper(t, func):
        ...
        result = t.label
        for b in t.branches:
            result = func(result, helper(b, func))
        return result

    if len(t.branches) == 2:
        max_left = helper(t.branches[0], max)
        min_right = helper(t.branches[1], min)  
        if max_left <= t.label and min_right > t.label:
    ...
    if len(t.branches) == 1:
        if t.branches[0].label <= t.label:
            ...
            min_right = helper(t.branches[0], min)
            if min_right > t.label:
                return is_bst(t.branches[0])
            else:
                return False
