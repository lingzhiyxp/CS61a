...
class Person:
    ...
    def __init__(self, name):
        self.name = name
        "*** YOUR CODE HERE ***"
        self.speak = 'I squirreled it away before it could catch on fire.'
...
class Round:
    ...
    def play(self, who, card):
        ...
        if card >= self.highest:
            ...
            self.now_winner = who
        if self.is_complete():
            self.winner = self.now_winner

    def is_complete(self):
        """ Checks if a game could end. """
        return self.next_player == self.starter and self.now_winner != None
