...
class VendingMachine:
    ...
    def restock(self, stock_num):
        self.current_num += stock_num
        return 'Current ' + self.good + ' stock: ' + str(self.current_num)
    
    def add_funds(self, fund):
        if self.current_num == 0:
            return 'Nothing left to vend. Please restock. Here is your $' + str(fund) + '.'
        else:
            self.current_balance += fund
    ...
    def vend(self):
        ...
        if self.current_balance < self.price:
            return 'Please add $' + str(self.price - self.current_balance) +' more funds.'
        if self.current_balance== self.price:
            self.current_num -= 1
            self.current_balance -= self.price
            return 'Here is your ' + self.good + '.'
        if self.current_balance > self.price:
            self.change = self.current_balance - self.price
            self.current_num -= 1
            self.current_balance = 0
            return 'Here is your ' + self.good + ' and $' + str(self.change) + ' change.'
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            self.previous = 0
            self.next_v = VirFib(1)
            self.next_v.previous = 0
        else:
            self.next_v = VirFib(self.previous + self.value)
            self.next_v.previous = self.value
        return self.next_v
