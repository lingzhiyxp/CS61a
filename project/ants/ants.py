        ...
        while not find_place.is_hive:
            if find_place.bees:
                if self.lower_bound <= distance <= self.upper_bound:
                    nearbee = random_bee(find_place.bees)
                    return nearbee
            find_place = find_place.entrance
            distance += 1
        return nearbee  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in attack_place.bees[:]:    # 为了防止原列表修改导致遍历出现问题
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while find_place != None:
            if find_place.ant:
                find_place.ant.double()
                if find_place.ant.is_container and find_place.ant.ant_contained:
                    find_place.ant.ant_contained.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not find_place.is_hive:
            if find_place.ant:
                if find_place.ant.is_container:
                    infront[find_place.ant] = distance
                    if find_place.ant.ant_contained and find_place.ant.ant_contained != self:
                        infront[find_place.ant.ant_contained] = distance
                else:
                    if find_place.ant != self:
                        infront[find_place.ant] = distance
            for bee in find_place.bees:
                infront[bee] = distance
            find_place = find_place.entrance
            distance += 1
        return infront
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        loss_by_distance = distance * 0.25
        loss_by_num = self.insects_shot * 0.0625
        return max(0.0, self.damage - loss_by_distance - loss_by_num)
        # END Problem Optional 2
